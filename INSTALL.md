...
## ⚠️ Важно о портах и доменах

**Сразу после установки:**
- ✅ Доступ через IP: `http://ваш-ip:8080` и `http://ваш-ip:3010`
- ❌ Доступ через домен: **НЕ РАБОТАЕТ** без настройки

**Для работы через домен нужно:**
1. Установить Nginx или Caddy (reverse proxy)
2. Настроить проксирование портов 8080→80 и 3010→443
3. Получить SSL сертификат (Let's Encrypt)

**Правильные примеры:**
- ✅ `http://192.168.1.100:8080` - доступ через IP
- ✅ `https://uwow-guide.online` - доступ через домен (после настройки Nginx/Caddy)
- ❌ `https://uwow-guide.online:8080` - **НЕПРАВИЛЬНО!** Домены не работают с портами напрямую

Инструкция по настройке домена: [Настройка HTTPS](#настройка-https-рекомендуется)
...
- Frontend: `http://ваш-ip:8080`
- GraphQL: `http://ваш-ip:3010/graphql`

**Если используете домен (нужен Nginx/Caddy):**
- Frontend: `https://ваш-домен.com` (через 443)
- GraphQL: `https://api.ваш-домен.com/graphql` (через 443)

⚠️ **Важно:** Домены работают только через 80/443 порты (нужен reverse proxy).
Прямой доступ к портам 3010/8080 - только через IP адрес!
